...
#对象是储存在内存中的实体，但我们并不能直接接触到该对象。我们在程序中的写的对象名，只是指向这一对象的引用(reference)。
#引用和对象分离，是动态类型的核心。引用可以随时指向一个新的对象：
a = 3       #"3"是存储在内存中的一个整数对象，通过赋值，引用a指向对象3
a = "at"    #"at"也是内存中的对象，是一个字符串。引用a指向"at"。此时，对象3不再有引用指向它。Pyhton会自动将没有引用指向的对象销毁，释放相应内存(但对于小的整数和短字符串，Python会进行缓存，避免了频繁的建立和销毁)。

a = 5
b = a       #a,b指向同一个整数对象5(b=a的含义是让引用b指向引用a所指向的那一个对象)
a = a + 2   #对引用a重新赋值，使引用a重新指向一个新的对象7
#多个引用指向同一个对象，如果一个引用值发生变化，那么实际上是让这个引用指向一个新的引用，并不影响其他的引用的指向。从效果上看，就是各个引用各自独立，互不影响。
#如下例子也是同样
L1 = [1, 2, 3]
L2 = L1
L1 = 1
...
L1[0] = 10
print L2
#L1,L2的指向并没有发生变化，依然指向那个list。list实际上是包含了多个引用对象(每个引用是一个元素，比如L1[0],L1[1]等，每个引用指向一个对象，比如1, 2, 3).
#而L1[0]=10，这一赋值操作，并没有改变L1的指向，而是对L1[0],也就是表对象的一部分(一个元素),进行操作，所以所有指向该对象的引用都受到了影响。
#列表(list)可以通过引用其元素，改变对象自身，这种对象类型称为："可变数据对象(mutable object)"，词典也是这样的数据类型。
#数字，字符串和元组(tuple)，不能改变对象本身，只能改变引用的指向，称为："不可变数据对象(immutable) object"
...
def f(x):
    ...
    print x
a = 1
f(a)        #参数x是一个新的引用，指向a所指的对象。如果参数是不可变的对象，a和x引用之间相互独立。对参数x的操作不会影响到a，这样的传递就类似于c中的"值传递"。
print a
...
def f(x):
    ...
    print x
a = [1, 2, 3]
f(a)        #传递的参数是可变的对象，那么改变函数参数，就可能改变原对象，此时指向原对象的引用都会受影响，编程时对此问题要注意。
print a
...
#2
#通过参数传递，判断数字、字符串、list、tuple、词典等数据类型是否为可变数据类型
#数字
a = 1
b = a
a = a + 1
...
print a, b
#dict
a = {"danhuiling": "yuanzai", "luozhe": "congmingzai" }
b = a
a["danhuiling"] = "pangzai"
print a, b
